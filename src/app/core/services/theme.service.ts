...
export class ThemeService {
  ...
  updateTheme(theme: string): void {
    ...
    const user: User | null = this._sessionService.getUser();

    if(user !== null) {
      user.settings.theme = theme;

      this._authService.updateUserSettings(user)
      .pipe(
        map( (success: boolean) => {
  ...
  setTheme(theme: string): void {
    ...
    const themeNode = this._document.getElementById('theme') as HTMLLinkElement;
